...
export default {
    ...
    login: async (req: Request, res: Response, next: NextFunction) => {
        try {
            ...
            let usuario = await UsuariosModel.findOne({ documento }).lean();
    ...
    getClienteByDocumento: async (req: Request, res: Response, next: NextFunction) => {
        try {
            let { documento } = req.query
            let usuario: any = null;
            if (!documento) throw new Error("Informe um documento");

            if (typeof documento === 'string') {
                isValidCPF(documento);
            } else {
                throw new Error("Documento deve ser uma string válida.");
            }
            usuario = await UsuariosModel.findOne({ documento }).lean();
            if (!usuario) throw new Error("Nenhum cliente encontrado!");
            if (!!usuario?.data_nascimento) {
                usuario.data_nascimento_form = dayjs(usuario.data_nascimento).format("YYYY-MM-DD");
            }
            res.json(usuario)
        } catch (error) {
            errorHandler(error, res);
    ...
    addUsuario: async (req: Request, res: Response, next: NextFunction) => {
        try {
            ...
            if (!req.body?._id) {
                if (!!req.body?.senha) {
                    payload.senha = bcrypt.hashSync(req.body.senha, 10);
                }
            }
    ...
    updateUsuarioAdicional: async (req: Request, res: Response, next: NextFunction) => {
        try {
            let doc;
            res.json(doc);
        } catch (error) {
